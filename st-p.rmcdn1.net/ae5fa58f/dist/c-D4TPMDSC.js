...
var M = g.extend({
        antialiasRotation: !0,
        render: function() {
            this.makeBox("widget-picture"), n("widget.fixMobileImageStretch") && this.el ? .classList.add("fixMobileImageStretch"), this.listenTo(this.page.mag, "change:viewport", this.onMagViewportBeforeChange), this.isLightboxEnabled() && this.initLightbox(), this.hasHoverAnimation && this.$el.addClass("has-onhover-animation"), this.rendered = !0, this.$lqipImg = null, this.useLqip = !h.isDownloadedSource && this.mag ? .viewOpts ? .lqip && !!this.blurHash && (h.useLambdaUrl(this.picture) || h.useLambdaUrlOldProjects(this.picture));
            let i = this.getPictureUrl();
            if (/\.gif/.test(i) && (this.isGIF = !0), this.$el.html(""), this.isVector(i) && !w.IS_FILE_PROTOCOL) this.loadVector(i);
            else {
                let e = !!(this.border_size > 0 || this.border_radius > 0 || this.border_radius_max || this.border_radius_independent && (this.border_radius_tl > 0 || this.border_radius_tr > 0 || this.border_radius_bl > 0 || this.border_radius_br > 0) || this.is_full_width || this.is_full_height),
                ...
                if (this.insideHotspot && this.isGIF) {
                    ...
                    let s = U(this.border_color || "000000", typeof this.border_color_opacity > "u" ? 1 : this.border_color_opacity / 100),
                        o = `rgba(${s.r}, ${s.g}, ${s.b}, ${s.a})`;
                ...
                if (h.useLambdaUrl(this.picture) || h.useLambdaUrlOldProjects(this.picture)) {
                    let s = h.getSrcset(this.picture, { ...this.getPictureOptions()
                    });
                    (0, r.default)("<img/>").attr("srcset", s).attr("alt", this.getAltText()).attr("src", i).addClass("viewable").toggleClass("saveable", !!(e || this.insideHotspot && this.isGIF)).css("opacity", 0).load(l.default.bind(function() {
                        let o = this.$el.find(".viewable"),
                            p = o && o.get(0) && o.get(0).currentSrc || i,
                            a = this.$el.find(".bgpic");
                        a && a.css({
                            opacity: t,
                            "background-image": `url(${p})`
                        }).attr("url", p), o && o.css("opacity", t), this.$lqipImg && this.$lqipImg.css("opacity", 0), l.default.delay(l.default.bind(function() {
                            this.$el.css({
                                overflow: "",
                                "border-radius": ""
                        ...
                        "background-image": `url("${this.blurHash?h.decodeBlurHash(this.blurHash,this.w,this.h):h.getLqipUrl(this.picture,{cropX:this.cropX,cropY:this.cropY,cropW:this.cropW,cropH:this.cropH,originalW:this.originalW,originalH:this.originalH})}")`,
                        "background-position": "50% 50%",
                        opacity: t
                    }).appendTo(this.$el))
                } else(0, r.default)("<img/>").appendTo(this.$el).toggleClass("saveable", !!(e || this.insideHotspot && this.isGIF)).css("object-fit", this.isGIF ? "cover" : "fill").css("opacity", typeof this.opacity > "u" ? 1 : this.opacity).addClass("viewable").load(l.default.bind(function() {
        ...
        isLightboxEnabled: function() {
            let i = !!this.getAnimations().find(t => t.type === "click" && t.steps && t.steps.length),
                e = !!this.clickLink;
            return this.lightbox ? this.lightbox.enabled && !i && !e : this.preview_enabled && !i && !e
        },
        isValid: function() {
        ...
        isVector: function(i, e) {
            let t = i || this.picture && this.picture.url;
            return (e || this.picture && this.picture.type) === "svg" ? !0 : t && l.default.last(t.split(".")).indexOf("svg") === 0
        },
        start: function() {
            return g.prototype.start.call(this), Modernizr.safari && this.isGIF && this.$el.flashClass("fix-safari-gif", 100), this
        },
        getPictureOptions: function() {
            let i = this.getAnimations().reduce((e, t) => Math.max(e, v.maxScaleAnimation(t)), 0);
            return {
                width: this.w,
                height: this.h,
                cropX: n("widget.fixMobileImageStretch") ? this.cropX ? ? this.originalCropX : this.cropX,
                cropY: n("widget.fixMobileImageStretch") ? this.cropY ? ? this.originalCropY : this.cropY,
                cropW: n("widget.fixMobileImageStretch") ? this.cropW ? ? this.originalCropW : this.cropW,
                cropH: n("widget.fixMobileImageStretch") ? this.cropH ? ? this.originalCropH : this.cropH,
                originalW: this.originalW,
                originalH: this.originalH,
                scale: u.isScaleLayout(this.mag.viewOpts) ? this.page.scale : 1,
        ...
        getSrcset: function(i = !0) {
            ...
            if (u.isOn(this.page.pageViewport, this.mag.getViewportWidth(this.page.pageViewport), this.mag.viewOpts)) return [];
            ...
            let s = [`${this.picture.finalUrl||this.picture.unscaledOptimizedUrl||this.picture.unscaledUrl} 1x`];
            return this.picture.final2xUrl && s.push(`${this.picture.final2xUrl} 2x`), this.picture.final2xUrl && (this.picture.unscaledOptimizedUrl || this.picture.final3xUrl) && s.push(`${this.picture.final2xUrl&&(this.picture.unscaledOptimizedUrl||this.picture.final3xUrl)} 3x`), s.concat(t)
        },
        loadVector: function(i) {
            this.$el.addClass("svg"), this.svgAjax = r.default.get(i + "?c", l.default.bind(function(e) {
                let t = (0, r.default)($.cleanupSVG((0, r.default)(e).find("svg").get(0).cloneNode(!0))),
                    s = t.get(0).getAttribute("viewBox");
                t.get(0).removeAttribute("width"), t.get(0).removeAttribute("height"), this.$svg = t;
                let o = t.get(0).cloneNode(!0);
                ...
                (0, r.default)(a).on("load error", l.default.bind(function() {
                    ...
                    let f = this.flags ? .useOriginalViewBoxSVG,
                        c = (0, r.default)(o);
                    f && s || c.get(0).setAttribute("viewBox", "0 0 " + (0, r.default)(a).width() + " " + (0, r.default)(a).height()), c.appendTo(window.document.body);
                    let d = c.get(0).getBBox();
                    f && s || this.$svg.get(0).setAttribute("viewBox", d.x + " " + d.y + " " + d.width + " " + d.height), this.$svg.get(0).setAttribute("preserveAspectRatio", "xMidYMid meet"), this.$svg.get(0).setAttribute("role", "img");
                    let b = this.getAltText();
                    b && this.$svg.get(0).setAttribute("aria-label", b), (0, r.default)(a).remove(), c.remove();
                    let m = (0, r.default)('<div class="svg-scale-wrapper"></div>');
                    this.$svg.appendTo(m), m.appendTo(this.$el), this.widgetIsLoaded()
                }, this)), a.src = i
        ...
        checkLightboxVisibility: function(i) {
            if (i && (this ? .page ? .lightboxModel ? .get("isVisible") || !1)) {
                let t = this ? .page ? .lightboxModel ? .get("onClose");
                t && typeof t == "function" && t()
            }
        },
        onMagViewportBeforeChange: function(i) {
            let e = i ? .viewport;
            this.checkLightboxVisibility(e)
        }
    }),
    G = M;
export {
