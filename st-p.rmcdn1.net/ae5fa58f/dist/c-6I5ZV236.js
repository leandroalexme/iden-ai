...
var G = M(S => {
    ...
    var f = function(t, e, i) {
        this._byteOffset = e || 0, t instanceof ArrayBuffer ? this.buffer = t : typeof t == "object" ? (this.dataView = t, e && (this._byteOffset += e)) : this.buffer = new ArrayBuffer(t || 0), this.position = 0, this.endianness = i ? ? f.LITTLE_ENDIAN
    };
    f.prototype = {};
    ...
    f.prototype.readInt32Array = function(t, e) {
        ...
        return f.memcpy(i.buffer, 0, this.buffer, this.byteOffset + this.position, t * i.BYTES_PER_ELEMENT), f.arrayToNative(i, e ? ? this.endianness), this.position += i.byteLength, i
    };
    ...
    f.flipArrayEndianness = function(t) {
        for (var e = new Uint8Array(t.buffer, t.byteOffset, t.byteLength), i = 0; i < t.byteLength; i += t.BYTES_PER_ELEMENT)
            for (var s = i + t.BYTES_PER_ELEMENT - 1, n = i; s > n; s--, n++) {
                var a = e[n];
                e[n] = e[s], e[s] = a
            }
        return t
    ...
    f.prototype.writeInt32Array = function(t, e) {
        if (this._realloc(t.length * 4), t instanceof Int32Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapInt32Array(t.length, e);
        else
            for (var i = 0; i < t.length; i++) this.writeInt32(t[i], e)
    };
    f.prototype.writeInt16Array = function(t, e) {
        if (this._realloc(t.length * 2), t instanceof Int16Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapInt16Array(t.length, e);
        else
            for (var i = 0; i < t.length; i++) this.writeInt16(t[i], e)
    };
    f.prototype.writeInt8Array = function(t) {
        if (this._realloc(t.length * 1), t instanceof Int8Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapInt8Array(t.length);
        else
            for (var e = 0; e < t.length; e++) this.writeInt8(t[e])
    };
    f.prototype.writeUint32Array = function(t, e) {
        if (this._realloc(t.length * 4), t instanceof Uint32Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapUint32Array(t.length, e);
        else
            for (var i = 0; i < t.length; i++) this.writeUint32(t[i], e)
    };
    f.prototype.writeUint16Array = function(t, e) {
        if (this._realloc(t.length * 2), t instanceof Uint16Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapUint16Array(t.length, e);
        else
            for (var i = 0; i < t.length; i++) this.writeUint16(t[i], e)
    };
    f.prototype.writeUint8Array = function(t) {
        if (this._realloc(t.length * 1), t instanceof Uint8Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapUint8Array(t.length);
        else
            for (var e = 0; e < t.length; e++) this.writeUint8(t[e])
    };
    f.prototype.writeFloat64Array = function(t, e) {
        if (this._realloc(t.length * 8), t instanceof Float64Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapFloat64Array(t.length, e);
        else
            for (var i = 0; i < t.length; i++) this.writeFloat64(t[i], e)
    };
    f.prototype.writeFloat32Array = function(t, e) {
        if (this._realloc(t.length * 4), t instanceof Float32Array && this.byteOffset + this.position % t.BYTES_PER_ELEMENT === 0) f.memcpy(this._buffer, this.byteOffset + this.position, t.buffer, 0, t.byteLength), this.mapFloat32Array(t.length, e);
        else
    ...
    f.prototype.mapInt32Array = function(t, e) {
        ...
        return f.arrayToNative(i, e ? ? this.endianness), this.position += t * 4, i
    };
    f.prototype.mapInt16Array = function(t, e) {
        this._realloc(t * 2);
        var i = new Int16Array(this._buffer, this.byteOffset + this.position, t);
        return f.arrayToNative(i, e ? ? this.endianness), this.position += t * 2, i
    };
    ...
    f.prototype.mapFloat64Array = function(t, e) {
        ...
        return f.arrayToNative(i, e ? ? this.endianness), this.position += t * 8, i
    };
    ...
    b.prototype.findEndContiguousBuf = function(t) {
        var e, i, s, n = t !== void 0 ? t : this.bufferIndex;
        if (i = this.buffers[n], this.buffers.length > n + 1)
            for (e = n + 1; e < this.buffers.length && (s = this.buffers[e], s.fileStart === i.fileStart + i.byteLength); e++) i = s;
    ...
    var k = function() {
        ...
        return this.parseOneDescriptor = function(h) {
            ...
            for (var d = 0; d < this.descs.length; d++)
                if (this.descs[d].tag == h) return this.descs[d];
            return null
    ...
    r.parseOneBox = function(t, e, i) {
        ...
        if (d == 1) {
            ...
            if (i) d = i;
            else if (u !== "mdat") return l.error("BoxParser", "Unlimited box size not supported for type: '" + u + "'"), s = new r.Box(u, d), {
                code: r.OK,
                box: s,
                size: s.size
            }
    ...
    r.ContainerBox.prototype.parse = function(t) {
        for (var e, i; t.getPosition() < this.start + this.size;)
            if (e = r.parseOneBox(t, !1, this.size - (t.getPosition() - this.start)), e.code === r.OK)
                if (i = e.box, this.boxes.push(i), this.subBoxNames && this.subBoxNames.indexOf(i.type) != -1) this[this.subBoxNames[this.subBoxNames.indexOf(i.type)] + "s"].push(i);
                else {
    ...
    r.createBoxCtor("av1C", function(t) {
        ...
        if (this.initial_presentation_delay_present = s >> 4 & 1, this.initial_presentation_delay_present === 1) this.initial_presentation_delay_minus_one = s & 15;
        else if (this.reserved_2 = s & 15, this.reserved_2 !== 0) {
    ...
    r.createFullBoxCtor("dref", function(t) {
        ...
        for (var s = t.readUint32(), n = 0; n < s; n++)
            if (e = r.parseOneBox(t, !1, this.size - (t.getPosition() - this.start)), e.code === r.OK) i = e.box, this.entries.push(i);
            else return
    });
    ...
    r.createFullBoxCtor("iinf", function(t) {
        ...
        for (var i = 0; i < this.entry_count; i++)
            if (e = r.parseOneBox(t, !1, this.size - (t.getPosition() - this.start)), e.code === r.OK) e.box.type !== "infe" && l.error("BoxParser", "Expected 'infe' box, got " + e.box.type), this.item_infos[i] = e.box;
            else return
    });
    ...
    r.createSampleGroupCtor("rash", function(t) {
        if (this.operation_point_count = t.readUint16(), this.description_length !== 2 + (this.operation_point_count === 1 ? 2 : this.operation_point_count * 6) + 9) l.warn("BoxParser", "Mismatch in " + this.grouping_type + " sample group length"), this.data = t.readUint8Array(this.description_length - 2);
        else {
    ...
    r.createFullBoxCtor("stsz", function(t) {
        ...
        if (this.sample_sizes = [], this.version === 0)
            for (this.sample_size = t.readUint32(), this.sample_count = t.readUint32(), e = 0; e < this.sample_count; e++) this.sample_size === 0 ? this.sample_sizes.push(t.readUint32()) : this.sample_sizes[e] = this.sample_size
    });
    r.createFullBoxCtor("stts", function(t) {
        var e, i, s;
        if (e = t.readUint32(), this.sample_counts = [], this.sample_deltas = [], this.version === 0)
            for (i = 0; i < e; i++) this.sample_counts.push(t.readUint32()), s = t.readInt32(), s < 0 && (l.warn("BoxParser", "File uses negative stts sample delta, using value 1 instead, sync may be lost!"), s = 1), this.sample_deltas.push(s)
    });
    r.createFullBoxCtor("stvi", function(t) {
        ...
        for (this.boxes = []; t.getPosition() < this.start + this.size;)
            if (s = r.parseOneBox(t, !1, this.size - (t.getPosition() - this.start)), s.code === r.OK) n = s.box, this.boxes.push(n), this[n.type] = n;
            else return
    });
    ...
    r.createFullBoxCtor("stz2", function(t) {
        var e, i, s;
        if (this.sample_sizes = [], this.version === 0)
            if (this.reserved = t.readUint24(), this.field_size = t.readUint8(), s = t.readUint32(), this.field_size === 4)
                for (e = 0; e < s; e += 2) {
                    var n = t.readUint8();
        ...
        else l.error("BoxParser", "Error in length field in stz2 box")
    });
    r.createFullBoxCtor("subs", function(t) {
        var e, i, s, n;
        for (s = t.readUint32(), this.entries = [], e = 0; e < s; e++) {
            var a = {};
            if (this.entries[e] = a, a.sample_delta = t.readUint32(), a.subsamples = [], n = t.readUint16(), n > 0)
                for (i = 0; i < n; i++) {
                    var o = {};
    ...
    r.createFullBoxCtor("trep", function(t) {
        for (this.track_ID = t.readUint32(), this.boxes = []; t.getPosition() < this.start + this.size;)
            if (ret = r.parseOneBox(t, !1, this.size - (t.getPosition() - this.start)), ret.code === r.OK) box = ret.box, this.boxes.push(box);
            else return
    });
    ...
    r.createFullBoxCtor("trun", function(t) {
        ...
        if (this.sample_count = t.readUint32(), e += 4, this.size - this.hdr_size > e && this.flags & r.TRUN_FLAGS_DATA_OFFSET ? (this.data_offset = t.readInt32(), e += 4) : this.data_offset = 0, this.size - this.hdr_size > e && this.flags & r.TRUN_FLAGS_FIRST_FLAG ? (this.first_sample_flags = t.readUint32(), e += 4) : this.first_sample_flags = 0, this.sample_duration = [], this.sample_size = [], this.sample_flags = [], this.sample_composition_time_offset = [], this.size - this.hdr_size > e)
            for (var i = 0; i < this.sample_count; i++) this.flags & r.TRUN_FLAGS_DURATION && (this.sample_duration[i] = t.readUint32()), this.flags & r.TRUN_FLAGS_SIZE && (this.sample_size[i] = t.readUint32()), this.flags & r.TRUN_FLAGS_FLAGS && (this.sample_flags[i] = t.readUint32()), this.flags & r.TRUN_FLAGS_CTS_OFFSET && (this.version === 0 ? this.sample_composition_time_offset[i] = t.readUint32() : this.sample_composition_time_offset[i] = t.readInt32())
    });
    ...
    r.sttsBox.prototype.unpack = function(t) {
        var e, i, s;
        for (s = 0, e = 0; e < this.sample_counts.length; e++)
            for (i = 0; i < this.sample_counts[e]; i++) s === 0 ? t[s].dts = 0 : t[s].dts = t[s - 1].dts + this.sample_deltas[e], s++
    };
    ...
    r.stscBox.prototype.unpack = function(t) {
        var e, i, s, n, a;
        for (n = 0, a = 0, e = 0; e < this.first_chunk.length; e++)
            for (i = 0; i < (e + 1 < this.first_chunk.length ? this.first_chunk[e + 1] : 1 / 0); i++)
                for (a++, s = 0; s < this.samples_per_chunk[e]; s++) {
                    if (t[n]) t[n].description_index = this.sample_description_index[e], t[n].chunk_index = a;
                    else return;
                    n++
                }
    };
    ...
    r.boxEqualFields = function(t, e) {
        ...
        var i;
        for (i in t)
            if (!(r.DIFF_BOXES_PROP_NAMES.indexOf(i) > -1)) {
...
`, o += d.payl.text
    ...
    N.prototype.parseSample = function(t) {
        ...
        if (!t.subsamples || t.subsamples.length === 0) e.documentString = s.readString(t.data.length);
            ...
            for (i = 1; i < t.subsamples.length; i++) e.resources[i] = s.readUint8Array(t.subsamples[i].size);
    ...
    c.prototype.getInfo = function() {
        var t, e, i = {},
            s, n, a, o, h = new Date("1904-01-01T00:00:00Z").getTime();
        if (this.moov)
            for (i.hasMoov = !0, i.duration = this.moov.mvhd.duration, i.timescale = this.moov.mvhd.timescale, i.isFragmented = this.moov.mvex != null, i.isFragmented && this.moov.mvex.mehd && (i.fragment_duration = this.moov.mvex.mehd.fragment_duration), i.isProgressive = this.isProgressive, i.hasIOD = this.moov.iods != null, i.brands = [], i.brands.push(this.ftyp.major_brand), i.brands = i.brands.concat(this.ftyp.compatible_brands), i.created = new Date(h + this.moov.mvhd.creation_time * 1e3), i.modified = new Date(h + this.moov.mvhd.modification_time * 1e3), i.tracks = [], i.audioTracks = [], i.videoTracks = [], i.subtitleTracks = [], i.metadataTracks = [], i.hintTracks = [], i.otherTracks = [], t = 0; t < this.moov.traks.length; t++) {
                if (s = this.moov.traks[t], o = s.mdia.minf.stbl.stsd.entries[0], n = {}, i.tracks.push(n), n.id = s.tkhd.track_id, n.name = s.mdia.hdlr.name, n.references = [], s.tref)
                    for (e = 0; e < s.tref.boxes.length; e++) a = {}, n.references.push(a), a.type = s.tref.boxes[e].type, a.track_ids = s.tref.boxes[e].track_ids;
    ...
    c.prototype.seekTrack = function(t, e, i) {
        ...
        for (s = 0; s < i.samples.length; s++) {
            if (n = i.samples[s], s === 0) h = 0, d = n.timescale;
    ...
    c.prototype.addTrack = function(t) {
        ...
        if (r[e.type + "SampleEntry"] !== void 0) {
            ...
            var o = "";
            for (var h in r.sampleEntryCodes)
                for (var d = r.sampleEntryCodes[h], u = 0; u < d.length; u++)
    ...
    c.initSampleGroups = function(t, e, i, s, n) {
        ...
        for (e && (e.sample_groups_info = []), t.sample_groups_info || (t.sample_groups_info = []), o = 0; o < i.length; o++) {
            ...
            if (n)
                for (a = 0; a < n.length; a++) n[a].grouping_type === i[o].grouping_type && (d.fragment_description = n[a], d.fragment_description.used = !0, d.is_fragment = !0)
        }
        if (e) {
            if (n)
                for (o = 0; o < n.length; o++) !n[o].used && n[o].version >= 2 && (u = n[o].grouping_type + "/0", d = new p(n[o].grouping_type, 0), d.is_fragment = !0, e.sample_groups_info[u] || (e.sample_groups_info[u] = d))
        } else
            for (o = 0; o < s.length; o++) !s[o].used && s[o].version >= 2 && (u = s[o].grouping_type + "/0", d = new p(s[o].grouping_type, 0), t.sample_groups_info[u] || (t.sample_groups_info[u] = d))
    };
    ...
    c.prototype.updateSampleLists = function() {
        ...
        if (this.moov !== void 0) {
            for (; this.lastMoofIndex < this.moofs.length;)
                if (d = this.moofs[this.lastMoofIndex], this.lastMoofIndex++, d.type == "moof")
                    for (u = d, t = 0; t < u.trafs.length; t++) {
    ...
    c.prototype.getSample = function(t, e) {
        ...
        if (!s.data) s.data = new Uint8Array(s.size), s.alreadyRead = 0, this.samplesDataSize += s.size, l.debug("ISOFile", "Allocating sample #" + e + " on track #" + t.tkhd.track_id + " of size " + s.size + " (total: " + this.samplesDataSize + ")");
    ...
    c.prototype.flattenItemInfo = function() {
        ...
        if (n != null && n.hdlr !== void 0 && n.iinf !== void 0) {
            ...
            if (n.pitm && (t[n.pitm.item_id].primary = !0), n.iref)
            ...
            if (n.iprp)
                for (var d = 0; d < n.iprp.ipmas.length; d++) {
    ...
    c.prototype.getItem = function(t) {
        ...
        if (i = this.items[t], !i.data && i.size) i.data = new Uint8Array(i.size), i.alreadyRead = 0, this.itemsDataSize += i.size, l.debug("ISOFile", "Allocating item #" + t + " of size " + i.size + " (total: " + this.itemsDataSize + ")");
        ...
        for (var s = 0; s < i.extents.length; s++) {
            ...
            if (n.alreadyRead !== n.length) {
                ...
                if (a > -1) {
                    ...
                    if (n.length - n.alreadyRead <= o) l.debug("ISOFile", "Getting item #" + t + " extent #" + s + " data (alreadyRead: " + n.alreadyRead + " offset: " + (n.offset + n.alreadyRead - e.fileStart) + " read size: " + (n.length - n.alreadyRead) + " full extent size: " + n.length + " full item size: " + i.size + ")"), f.memcpy(i.data.buffer, i.alreadyRead, e, n.offset + n.alreadyRead - e.fileStart, n.length - n.alreadyRead), e.usedBytes += n.length - n.alreadyRead, this.stream.logBufferLevel(), i.alreadyRead += n.length - n.alreadyRead, n.alreadyRead = n.length;
                    else return l.debug("ISOFile", "Getting item #" + t + " extent #" + s + " partial data (alreadyRead: " + n.alreadyRead + " offset: " + (n.offset + n.alreadyRead - e.fileStart) + " read size: " + o + " full extent size: " + n.length + " full item size: " + i.size + ")"), f.memcpy(i.data.buffer, i.alreadyRead, e, n.offset + n.alreadyRead - e.fileStart, o), n.alreadyRead += o, i.alreadyRead += o, e.usedBytes += o, this.stream.logBufferLevel(), null
                } else return null
            }
        }
