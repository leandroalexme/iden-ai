...
var V, D, Q, K = H(() => {
    ...
    D.prototype = {
        ...
        generateShapeSVG: function(b, l, w, q, k, x = {}) {
            let $ = 1e5,
                G = x ? .bg_color || l.get("bg_color") || l.viewport_default && l.viewport_default.bg_color || "ffffff",
                B = x ? .bg_opacity || l.get("bg_opacity"),
                X = x ? .color || l.get("color") || "ffffff",
                v = x ? .opacity || l.get("opacity"),
                R = x ? .borders || l.get("borders"),
                j = x ? .radius || l.get("radius") || 0,
                _ = x.w || w,
                I = x.h || q,
                d = this.getRGBA(G, B),
                L = this.getRGBA(X, v),
                M;
            if (l.get("type") === "hotspot" ? k = k || x ? .pin_type || l.get("pin_type") : k = k || l.get("tp"), k == "ellipse") {
                var S = _ / 2,
                    p = I / 2,
                    C = R,
                    C = Math.min(C, S, p);
            ...
            if (k == "rectangle" || k == "polygon") {
                ...
                let t = k == "rectangle" ? 4 : l.get("sides"),
                    ...
                    i = O(n),
                    r, o;
                k === "rectangle" && h ? (r = Z(n, f, i), o = Y(f, r)) : (r = N(n, j, i), o = F(n, r.points, r.radius));
            ...
            if (k == "line") {
                ...
                if (h = "M" + r + " " + e + " L" + _ + " " + e, l.get("stroke") == "solid" && (M = `<path d="${h}" style="fill: none; stroke: ${d}; stroke-width: ${t}" shape-rendering="${i}" />`), l.get("stroke") == "double") {
                    let o = "M" + r + " " + (e - t) + " L" + _ + " " + (e - t),
                        s = "M" + r + " " + (e + t) + " L" + _ + " " + (e + t);
                    M = `<path d="${o}" style="fill: none; stroke: ${d}; stroke-width: ${t};" shape-rendering="${i}"/>`, M += `<path d="${s}" style="fill: none; stroke: ${d}; stroke-width: ${t}" shape-rendering="${i}"/>`
                }
                l.get("stroke") == "dotted" && (M = `<path d="${h}" stroke-linecap="round" stroke-dasharray="${n}" style="fill: none; stroke: ${d}; stroke-width: ${t}"  shape-rendering="${i}"/>`), l.get("stroke") == "dashed" && (M = `<path d="${h}" stroke-dasharray="${f}" style="fill: none; stroke: ${d}; stroke-width: ${t}" shape-rendering="${i}"/>`)
            }
            if (k == "icon") {
            ...
            function O(t) {
                ...
                for (let n = 0; n < h; n++) {
                    let i = t[n].x - t[(n + 1) % h].x,
                        r = t[n].y - t[(n + 1) % h].y;
                    f = Math.sqrt(i * i + r * r), f < e && (e = f), t[n].side = f
                }
                return e
            }
            ...
            function Z(t, {
                ...
                return e = Math.min(e, i / 2), h = Math.min(h, i / 2), f = Math.min(f, i / 2), n = Math.min(n, i / 2), {
                    trTop: {
                        x: t[0].x - e,
                        y: t[0].y
                    },
            ...
            function N(t, e, h) {
                ...
                for (let A = 0; A < n; A++) {
                    var i = t[A],
                        ...
                        c = (r.y - i.y) / o,
                        y, u, a, g;
                    y = i.x + s * e, u = i.y + c * e, a = r.x - s * e, g = r.y - c * e, y = Math.round(y * $) / $, u = Math.round(u * $) / $, a = Math.round(a * $) / $, g = Math.round(g * $) / $, f.push({
                        x: y,
                        y: u
                    }), f.push({
                        x: a,
