...
var T, O, A, w, q, S, _ = l(() => {
    "use strict";
    E();
    T = (s, e) => p.post(`/api/ecommerce/${s}/stripe/checkout-session`, { ...e
    }), O = (s, e) => p.get(`/api/ecommerce/${s}/stripe/customer/by-checkout-session/${e}`), A = (s, e) => p.post(`/api/ecommerce/${s}/stripe/billing-portal-session`, { ...e
...
var h, b, v = l(() => {
    ...
    h = ["SELECT", "SIZE", "RADIO"], b = class {
        ...
        async loadProducts(e, r, t) {
            let c = t && window ? .RM ? .screenshotMode === "preview" ? "?type=preview" : r ? "?type=published" : "";
        ...
        productHasActiveOptions(e) {
            return !!e.options ? .some(r => h.includes(r.type))
        }
        productHasActivePrice(e) {
            return Array.isArray(e.prices) && !!e.prices ? .some(r => r.active)
        }
        ...
        getProductForAddToCart(e, r) {
            ...
            if (t && r && (t.selectedOptions = { ...r
                }, Array.isArray(t.combinations))) {
                let c = t.combinations.find(n => n.options.every(o => r[o.name] === o.value));
                c && (t.selectedCombination = { ...c
                })
            }
            return t
        }
        initFrontendLibrary(e) {
            if (!this.libraryAdded && e ? .ecwid ? .serviceData) {
                let r = document.createElement("script"),
                    t = `https://app.ecwid.com/script.js?${e.ecwid.serviceData.storeId}&data_platform=code`;
                r.setAttribute("async", "async"), r.setAttribute("charset", "utf-8"), r.setAttribute("src", t), document ? .body ? .appendChild(r), this.libraryAdded = !0
            }
        }
                ...
                if (t.options) {
                    for (let a in r) {
                        let u = t.options.find(d => d.name === a),
                            i = u ? u.choices ? .find(d => d.text === r[a]) : null;
                    ...
                    n.optionsTranslated = Object.fromEntries(Object.entries(n.options).map(([a, u]) => {
                        let i = t.options ? .find(m => m.name === a);
                        if (!i) return [a, u];
                        let d = y(i);
        ...
        async goToCheckout({
            ...
            o.init(), await new Promise(i => o.Cart.clear(i)), await new Promise(i => o.Cart.calculateTotal(i));
...
export {
    f as a, T as b, O as c, A as d, w as e, q as f, _ as g, h, b as i, v as j
};