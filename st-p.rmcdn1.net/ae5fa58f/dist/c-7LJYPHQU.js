import {
    ...
    d as E,
    g as I
} from "https://st-p.rmcdn1.net/ae5fa58f/dist/c/c-HU7KMF5A.js";
import {
    a as K
} from "https://st-p.rmcdn1.net/ae5fa58f/dist/c/c-ZAUIFYIX.js";
import {
    a as w,
    p as G
    ...
    g = class {
        onMagFullLoaded(e) {
            ...
            if (window.history.replaceState({}, document.title, r.pathname + (h ? `?${h}` : "")), i && i === "success" && (e.clearCart(), e.openSidebarOnRender = {
                    checkoutComplete: !0
                }, s && u === "subscription" && T(e.mag.num_id, s).then(c => c.data).then(c => c ? .id && E(e.mag.num_id, {
                    customer: c.id,
                    return_url: window.location.href.split("?")[0]
                })).then(({
                    data: c
                }) => {
                    c ? .url && o.trigger("ecommerce:billingPortalUrl:get", c.url)
                }), "sessionStorage" in window)) try {
                let c = "stripePurchaseEvent",
                    m = window.sessionStorage.getItem(c);
                if (m) {
                    let F = JSON.parse(m),
                        b = setInterval(() => {
        ...
        productHasActiveSku(e) {
            return Array.isArray(e ? .articles) && e.articles.some(r => r.active)
        }
        ...
        getProductForAddToCart(e, r) {
            ...
            if (a) return a.productObject = { ...e,
                articles: null,
                prices: null
            }, a;
            let i = this.getSkuBySelectsValues(e, r);
            return i ? (i.productObject = { ...e,
                articles: null,
                prices: null
            }, i) : null
        }
...
var y, A = S(() => {
    ...
    y = class {
        ...
        productHasActiveSku(e) {
            return e ? .options && e.options.some(r => this.isOptionValid(r))
        }
        isOptionValid(e) {
            return ["SELECT", "SIZE", "RADIO"].includes(e ? .type)
        }
        mapProductsSelectList(e = [], r) {
            let a = e.map(i => ({
                value: i.id,
        ...
        countCartItemsById(e, r) {
            ...
            return Object.keys(r.prices).forEach(i => {
                let s = r.prices[i];
                s && (s.id === e || s.selectedCombination ? .id === e) && (a += s.cart_count || 0)
            }), a
        }
        isOutOfStock(e, r) {
            if (!e) return !0;
            ...
            return a.prices = {}, Object.keys(r.prices).forEach(i => {
                ...
                if (u) {
                    if (a.prices[i] = { ...s,
                        ...
                        let h = u.combinations.find(c => c.id === s.selectedCombination ? .id);
...
function R(t) {
    ...
    for (let i of t) {
        ...
        e += s * i.cart_count, r = i.currency.toUpperCase(), a.push({
            item_id: i.id,
            item_name: i.productObject ? .name,
            price: s,
...
var x, v, d, $, N, L, l, n, Q, C, pt, Y = S(() => {
    "use strict";
    x = f(H()), v = f(B()), d = f(q()), $ = f(K());
    M();
    J();
    _();
    V();
    ...
    v.default.extend(C.prototype, {
        ecommerceProviders: Q,
        CONNECTION_TIMEOUT: 5e3,
        stripeJsSrc: "https://js.stripe.com/v3/",
        isConstructor: function() {
            return this.environment === p.environment.constructor
        },
        isViewer: function() {
            return this.environment === p.environment.viewer
        },
        isPreview: function() {
            return this.environment === p.environment.preview
        },
        getStorageKey() {
        ...
        updateCartProductItems: async function() {
            let t = this.adapter ? .updateCartProductItems(this.products, this.cartData);
        ...
        getTextValue: function(t) {
            let r = this.getEcommerceData() ? .texts ? .[t];
            return r ? k.sanitize(r) : L[t]
        },
        ...
        initSocket: function() {
            return new Promise(function(t, e) {
                let r = window.location.origin === p.readymag_auth_host,
                    a = r && p.isProd ? "sockets.readymag.com/stripe-auth" : "/stripe-auth";
                P("constructor.socketsHostFromEnv") && (a = r && p.readymag_sockets_host ? `${p.readymag_sockets_host}/stripe-auth` : "/stripe-auth");
                let i = io(a, {
                    timeout: this.CONNECTION_TIMEOUT,
                    transports: ["websocket"]
                });
                i.on("connect", function() {
                    i.emit("wainting for mag stripe auth update", {
                        num_id: this.mag.get("num_id"),
                        user: this.router.me.pick("name", "email", "_id"),
                        hash: this.mag.get("socket_hash")
                    }), i.on("state-update", function(s) {
                        i.disconnect(), s.error ? e(s.error) : t(s.ecommerce_data)
                    })
                }.bind(this))
            }.bind(this))
        },
        ...
        isProductAvailable(t) {
            return this.adapter ? .isProductAvailable(t)
        },
        isOutOfStock(t) {
            return this.adapter ? .isOutOfStock(t, this.cartData)
        },
        getProductForAddToCart(t, e) {
            return this.adapter ? .getProductForAddToCart(t, e)
        },
        addProductToCart(t) {
            switch (t.object) {
                case "price":
        ...
        getCartItenKey(t) {
            return t.object === "product" && t.selectedOptions && Object.keys(t.selectedOptions).length > 0 ? (0, $.default)(`${t.id}-${JSON.stringify(t.selectedOptions)}`) : t.id
        },
        ...
        increaseSkuCartItem: function(t) {
            if (this.cartData.skus[t])
                if (this.checkItemQuantity(t)) this.cartData.skus[t].cart_count += 1, this.markCartChange(), this.backupCartData(), o.trigger("ecommerce:cartdata:changed", this.cartData);
                else return new n("You can\u2019t add more this product with attribute " + t + ". It's out of stock", l.outOfStock)
        },
        ...
        createCheckoutSession({
            ...
            if ("sessionStorage" in window) {
                let m = R(Object.values(this.cartData.prices));
                window.sessionStorage.setItem(a, JSON.stringify(m))
            }
            let i = !1,
                s = [];
            Object.keys(this.cartData.prices).forEach(m => {
                s.push({
                    price: this.cartData.prices[m].id,
                    quantity: this.cartData.prices[m].cart_count
                }), this.cartData.prices[m].type === "recurring" && (i = !0)
            });
            ...
            return !i && e && r && (c.shipping_rates = e, c.shipping_address_collection = r), O(this.mag.num_id, c)
        },
    ...
    C.hasEcommerceWidget = (t, e = []) => t === p.environment.constructor ? !!e.find(r => r.get("type") === "ecommercecart") : !!e.find(r => r.type === "ecommercecart");
    pt = C
});
export {
    l as a, n as b, pt as c, R as d, Y as e
};