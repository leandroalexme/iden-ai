        ...
        if (t.isDragging) {
            ...
            if (o) {
                let u = l * Math.cos(s(o)) + i * Math.sin(s(o)),
                    ...
                    f = (t.mouseStart ? .x || 0) * Math.cos(s(o)) + (t.mouseStart ? .y || 0) * Math.sin(s(o)),
                    S = -1 * (t.mouseStart ? .x || 0) * Math.sin(s(o)) + (t.mouseStart ? .y || 0) * Math.cos(s(o));
                r = (u - f) / n / (t.animationScale ? .x || 1), e = (m - S) / n / (t.animationScale ? .y || 1)
            } else r = (l - (t.mouseStart ? .x || 0)) / n / (t.animationScale ? .x || 1), e = (i - (t.mouseStart ? .y || 0)) / n / (t.animationScale ? .y || 1);
